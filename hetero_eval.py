...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        timesteps_list = np.random.normal(loc=args.timestep_mean, scale=args.timestep_std, size=args.num_users)
        print("User-decided training timesteps: ", [round(timesteps_list[idx]) for idx in chosen_users])

        # update global weights
        chosen_timesteps = [round(timesteps_list[idx]) for idx in chosen_users]
        if args.FedAvgWeight == "timestep_prop":
            ...
            inverse_chosen_timesteps = [1/(timestep**1.5) for timestep in chosen_timesteps]
        ...
        if iter % args.eval_every == 0:
            # testing
            net_glob.eval()
            avg_train_acc, avg_train_loss, avg_test_acc, avg_test_loss = [], [], [], []
            for ts in [15, 20, 25]:
                net_glob.module.set_timestep(ts)
                acc_train, loss_train = test_img(net_glob, dataset_train, args)
                print("Round {:d}, Timestep {}, Training accuracy: {:.2f}".format(iter+1, ts, acc_train))
                acc_test, loss_test = test_img(net_glob, dataset_test, args)
                print("Round {:d}, Timestep {}, Testing accuracy: {:.2f}".format(iter+1, ts, acc_test))
                avg_train_acc.append(acc_train)
                avg_train_loss.append(loss_train)
                avg_test_acc.append(acc_test)
                avg_test_loss.append(loss_test)

            acc_train = sum(avg_train_acc)/len(avg_train_acc)
            loss_train = sum(avg_train_loss)/len(avg_train_loss)
            acc_test = sum(avg_test_acc)/len(avg_test_acc)
            loss_test = sum(avg_test_loss)/len(avg_test_loss)
